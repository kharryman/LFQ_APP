		...
		edit.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (insert_definition.isChecked()) {
					...
					MainLfqActivity.getDictionaryDb().delete("dictionarya", "Word=?",
							new String[] { selwor });
					MainLfqActivity.getDictionaryDb().insert("dictionarya", null, values);
					sql = "DELETE FROM " + Helpers.db_prefix + "dictionary.dictionarya WHERE Word='"
							+ selwor + "'";
					sql2 = "INSERT INTO " + Helpers.db_prefix + "dictionary.dictionarya (Word,PartSpeech,Definition,Number) VALUES('"
							+ selwor
							+ "','"
							+ partspeech
							+ "','"
					...
					autosync_text = Synchronize.autoSync(sql, "dictionary_db",
							"delete", "dictionarya", selwor, false, null);
					autosync_text += Synchronize.autoSync(sql2,
							"dictionary_db", "insert", "dictionarya", selwor,
							false, null);
				...
				if (edit_definition.isChecked()) {
					...
					int number = getNumber(selwor);
					values.put("Number", number);
					MainLfqActivity.getDictionaryDb().update("dictionarya", values, "Word=?",
							new String[] { selwor });
					sql = "UPDATE " + Helpers.db_prefix + "dictionary.dictionarya SET PartSpeech='"
							+ partspeech
							+ "', Number='"
							+ number
							+ "', Definition='"
							+ definition
							+ "'  WHERE Word='" + selwor + "'";					
						autosync_text = Synchronize.autoSync(sql, "dictionary_db", "update", "dictionarya", selwor, false, null);					
				...
				if (delete_definition.isChecked()) {
					...
					sql = "DELETE FROM " + Helpers.db_prefix + "dictionary.dictionarya WHERE Word='"
							+ selwor + "'";
					autosync_text = Synchronize.autoSync(sql, "dictionary_db", "delete", "dictionarya", selwor, false, null);					
