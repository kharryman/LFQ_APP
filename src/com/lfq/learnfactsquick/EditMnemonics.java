...
public class EditMnemonics extends Activity {
	private LinearLayout scroll, add_remove_layout;
	private Spinner select_mnemonic_table;
	private TextView results;
	private EditText input_mnemonic_title, input_number_mnemonic_entries,
			input_mnemonic_table_name;
	private CheckBox check_insert_mnemonic_line_breaks,
			check_insert_new_mnemonic_table;
	private RadioButton check_insert_mnemonics, check_insert_number_mnemonics,
			check_insert_mnemonic_anagrams, check_insert_mnemonic_peglist;
	private Button do_begin_update_mnemonics, do_update_mnemonics,
			do_delete_mnemonics, do_delete_mnemonic_table, do_insert_mnemonics,
			do_begin_mnemonic_edit, do_get_mnemonic_entries;
	...
	public void saveChanges() {
		...
		editor.putBoolean("EDIT MNEMONICS CHECK NEW TABLE",
				check_insert_new_mnemonic_table.isChecked());
		...
		editor.putString("EDIT MNEMONICS TABLE INPUT",
				input_mnemonic_table_name.getText().toString());
	...
	public void setViews() {
		...
		// BUTTONS:
		do_begin_mnemonic_edit = (Button) findViewById(R.id.do_begin_mnemonic_edit);
		do_get_mnemonic_entries = (Button) findViewById(R.id.do_get_mnemonic_entries);
		do_update_mnemonics = (Button) findViewById(R.id.do_update_mnemonics);
		do_begin_update_mnemonics = (Button) findViewById(R.id.do_begin_update_mnemonics);
		do_delete_mnemonics = (Button) findViewById(R.id.do_delete_mnemonics);
		do_delete_mnemonic_table = (Button) findViewById(R.id.do_delete_mnemonic_table);
		do_insert_mnemonics = (Button) findViewById(R.id.do_insert_mnemonics);
		...
		check_insert_new_mnemonic_table = (CheckBox) findViewById(R.id.check_insert_new_mnemonic_table);
		...
		input_mnemonic_table_name = (EditText) findViewById(R.id.input_mnemonic_table_name);
	...
	public void loadButtons() {
		do_begin_mnemonic_edit.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_get_mnemonic_entries.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_update_mnemonics.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_begin_update_mnemonics.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_delete_mnemonics.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_delete_mnemonic_table.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
		do_insert_mnemonics.setBackgroundResource(sharedPref.getInt(
				"BG Button", R.drawable.button));
	...
	public void setListeners() {
		...
		do_begin_mnemonic_edit.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				add_remove_layout.setVisibility(View.GONE);
				if (input_number_mnemonic_entries.getText().toString()
						.equals("")) {
					results.setText("NEED TO ENTER 'Number Of Entries'\n");
				} else {
					num_entries = Integer
							.parseInt(input_number_mnemonic_entries.getText()
									.toString());
				}
				...
				for (int i = 0; i < num_entries; i++) {
					...
					word.get(i).setBackgroundResource(
							R.drawable.rounded_edittext_red);
					...
					info.get(i).setBackgroundResource(
							R.drawable.rounded_edittext_red);
		...
		do_get_mnemonic_entries.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				add_remove_layout.setVisibility(View.GONE);
				status = "get entries";
				...
				if (c_get.moveToFirst()) {
					...
					do {
						if (c_get.getColumnIndex(cat_sel) == -1) {
							continue;
						}
						RadioButton rad = new RadioButton(this_act);
						type = c_get.getString(c_get
								.getColumnIndex("Mnemonic_Type"));
						title = c_get.getString(c_get.getColumnIndex("Title"));
						rad.setText(title + "(" + type + ")");
						ent_num = Integer.parseInt(c_get.getString(c_get
								.getColumnIndex("Entry_Number")));
						rad.setId(ent_num);
						text_list.put(String.valueOf(ent_num), substr);
						rad.setOnClickListener(new View.OnClickListener() {
							@Override
							public void onClick(View v) {
								sav_ent_num = v.getId();
								results.setText("SELECTED "
										+ text_list.get(String
												.valueOf(sav_ent_num)) + ".");
							}
						});
						rad_ents.addView(rad);
					} while (c_get.moveToNext());
					c_get.close();
					scroll.addView(rad_ents, params);
					results.setText("Got entries for " + cat_sel);
				}

			}
		});
		do_begin_update_mnemonics
				.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						if (sav_ent_num == -1 || !status.equals("get entries")) {
							results.setText("NOTHING SELECTED");
							return;
						}
						status = "begin update";
						cat_sel = select_mnemonic_table.getSelectedItem()
								.toString();
						Cursor c_get1 = MainLfqActivity.getMneDb().rawQuery(
								"SELECT * FROM mnemonics WHERE Category='"
										+ cat_sel + "' AND Entry_Number='"
										+ sav_ent_num + "'", null);
						if (c_get1.moveToFirst()) {
							...
							input_mnemonic_title.setText(c_get1
									.getString(c_get1.getColumnIndex("Title")));
							...
							if (linebreak.equals("1")) {
								check_insert_mnemonic_line_breaks
										.setChecked(true);
							} else {
								check_insert_mnemonic_line_breaks
										.setChecked(false);
							...
							infospl.clear();
							do {
								wordspl.add(c_get1.getString(c_get1
										.getColumnIndex("Entry")));
		...
		do_update_mnemonics.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				add_remove_layout.setVisibility(View.GONE);
				if (!status.equals("begin update")) {
					results.setText("NEED TO BEGIN UPDATE FIRST.");
					return;
				}
				...
				if (c_get1.moveToFirst()) {
					...
					do {
						...
						sql = "UPDATE " + Helpers.db_prefix
								+ "dictionary.mnemonics SET Title='" + title + "', Entry='" + ent + "', Entry_Mnemonic='" + ent_mne + "' Entry_Info='" + ent_info + "' WHERE Category='" + cat_sel + "' AND Entry_Number='" + sav_ent_num + "' AND Entry_Index='" + j + "'";
				...
				MainLfqActivity.getMneDb().update("mnemonics", cv, "Category=? AND Entry_Number=?", new String[]{cat_sel,String.valueOf(sav_ent_num)});
		...
		do_delete_mnemonics.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				add_remove_layout.setVisibility(View.GONE);
				cat_sel = select_mnemonic_table.getSelectedItem().toString();
				...
				if (!status.equals("get entries")) {
					results.setText("NEED TO GET ENTRIES FIRST");
					return;
				}
				...
				// autoSync(sql, db, action, table, name, bool is_image, byte[]
				// image)
				autosync_text = Synchronize.autoSync(sql, "mne_db", "delete",
						cat_sel, String.valueOf(sav_ent_num), false, null);
				do_get_mnemonic_entries.performClick();
			}
		});
		do_delete_mnemonic_table.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				add_remove_layout.setVisibility(View.GONE);
				cat_sel = select_mnemonic_table.getSelectedItem().toString();
				...
				// autoSync(sql, db, action, table, name, bool is_image, byte[]
				// image)
				autosync_text += Synchronize.autoSync(sql, "mne_db",
						"delete_category", cat_sel, "", false, null);
				results.setText("DELETED table " + cat_sel + "."
						+ autosync_text);
				resetSelectTables();
			}
		});
		do_insert_mnemonics.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				// ENTER TITLE:
				add_remove_layout.setVisibility(View.GONE);
				if (!status.equals("begin input")) {
					results.setText("MUST BEGIN INPUTTING FIRST.");
					return;
				}				
				title = input_mnemonic_title.getText().toString();
				String type = "";
				...
				Cursor c_max = MainLfqActivity.getMneDb().rawQuery("SELECT MAX(Entry_Number) AS MAX_NUM FROM mnemonics WHERE Category=?", new String[]{cat_sel});
				int ent_num_max = 0; 
				...
				for (int i = 0; i < num_entries; i++) {
					...
					// SYNC TO LFQ DATABASE:
					sql = "INSERT INTO " + Helpers.db_prefix + "dictionary.mnemonics (Title,Type,Is_Linebreak,Entry,Entry_Mnemonic,Entry_Info,Entry_Number,Entry_Index)"
					       + " VALUES('" + title + "','" + type + "','" + linebreak + "','" + ent + "','" + ent_mne + "','" + ent_info + "','" + ent_num_max + "','" + j + "')";
					// autoSync(sql, db, action, table, name, bool is_image, byte[]
					// image)
				...
				Cursor c_mne_tabs = MainLfqActivity
						.getMneDb()
						.rawQuery(
	...
	class doLoadDatabases extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String file_url) {
			...
			check_insert_new_mnemonic_table.setChecked(sharedPref.getBoolean(
					"EDIT MNEMONICS CHECK NEW TABLE", false));
			...
			input_mnemonic_table_name.setText(sharedPref.getString(
			...
			if (sharedPref.getString("EDIT MNEMONICS STATUS", "").equals(
					"begin input")) {
				do_begin_mnemonic_edit.performClick();
				for (int i = 0; i < mnemonic.size(); i++) {
					if (mnemonic.get(i) != null) {
						mnemonic.get(i).setText(
								sharedPref.getString("EDIT MNEMONICS MNEMONIC "
										+ i, ""));
					}
				}
				for (int i = 0; i < word.size(); i++) {
					word.get(i).setText(
							sharedPref
									.getString("EDIT MNEMONICS WORD " + i, ""));
				}
				for (int i = 0; i < info.size(); i++) {
					info.get(i).setText(
							sharedPref
									.getString("EDIT MNEMONICS INFO " + i, ""));
				}
