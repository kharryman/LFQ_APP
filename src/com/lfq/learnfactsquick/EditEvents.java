...
public class EditEvents extends Activity {
			...
			public void onClick(View v) {
				...
				if (insert_event.isChecked()) {
					...
					if (check_shared_table.isChecked()) {
						...
						sql = "INSERT INTO "
								+ lfq_table
								+ " (Year,Date,Event,SaveWords,Acrostics,Type) VALUES('"
								+ year + "','" + date + "','" + event + "','"
								+ major_words + "','" + acrostics + "','"
								+ type + "')";
				...
				if (edit_event.isChecked()) {
					...
					if (!check_shared_table.isChecked()) {
						...
						sql = "UPDATE " + lfq_table + " SET Year='" + year
								+ "' Date='" + date + "', Event='" + event
								+ "', SaveWords='" + major_words
								+ "', Acrostics='" + acrostics + "', Type='"
								+ type + "' WHERE Year='" + year + "' AND ID='"
								+ id + "'";
					}
					// autoSync(sql, db, action, table, name, bool is_image,
					// byte[] image)
				...
				if (delete_event.isChecked()) {
					...
					MainLfqActivity.getDatabase().delete(table,
							"Year=? AND _id=?", selectionArgs);
					// SYNC TO LFQ:
					autosync_text = "";
					sql = "DELETE FROM " + lfq_table + " WHERE Date='" + date
							+ "' AND Year='" + year + "' AND ID='" + id + "'";
					// autoSync(sql, db, action, table, name, bool is_image,
					// byte[] image)
					autosync_text = Synchronize.autoSync(sql, database_string,
			...
			if (check_get_years.isChecked()) {
				...
				if (c_get_next.getCount() == 0) {
					...
					c_get_next = MainLfqActivity.getDatabase().rawQuery(
							"SELECT * FROM " + table + " WHERE Date>'" + date
									+ "' AND Year='" + year
									+ "' ORDER BY Date,_id LIMIT 1", null);
				}
			}
	...
	class doLoadSelectYears extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String file_url) {
			c = MainLfqActivity.getDatabase().rawQuery(
					"SELECT Date FROM " + table + " WHERE Year='"
	...
	public void doGetDates() {
		...
		Cursor c_first = MainLfqActivity
				.getDatabase()
				.rawQuery(
						"SELECT * FROM "
								+ table
		...
		System.out.println("table=" + table + ", date=" + date + ", year="
				+ year);
		c = MainLfqActivity.getDatabase().rawQuery(
				"SELECT DISTINCT Year FROM " + table + " WHERE Date='" + date
	...
	class doSelectYear extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (check_get_years.isChecked()) {
				c = MainLfqActivity.getDatabase().rawQuery(
						"SELECT Date FROM " + table + " WHERE Year='" + year
								+ "'", null);
				total = c.getCount();
				publishProgress("");
			}
