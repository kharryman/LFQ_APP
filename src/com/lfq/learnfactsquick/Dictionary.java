		...
		protected String doInBackground(String... params) {
			...
			if (is_one_word) {
				...
				Cursor c = MainLfqActivity.getDictionaryDb()
						.rawQuery(
								"SELECT Word,PartSpeech,Definition FROM dictionarya WHERE Word LIKE ? ORDER BY Word LIMIT 1000",
								selectionArgs);
			...
			if (!is_one_word) {
				...
				for (int i = 0; i < and_l; i++) {
					def_expression += " Definition LIKE '% " + and_spl[i]
							+ " %'";
					if (i < (and_l - 1)) {
						def_expression += " AND";
					}
				...
				for (int i = 0; i < or_l; i++) {
					...
					def_expression += " Definition LIKE '% " + or_spl[i]
							+ " %'";
					if (i < (or_l - 1)) {
						def_expression += " OR";
					}
				...
				for (int i = 0; i < nor_l; i++) {
					...
					def_expression += " Definition NOT LIKE '% " + nor_spl[i]
							+ " %'";
					if (i < (nor_l - 1)) {
						def_expression += " AND";
					}
				...
				String query = "SELECT Word,PartSpeech,Definition FROM dictionarya WHERE"
						+ def_expression
						+ " ORDER BY Word COLLATE NOCASE LIMIT 300";
				...
				if (c_seadic.moveToFirst()) {
					...
					do {
						beg_let = c_seadic
								.getString(c_seadic.getColumnIndex("Word"))
