...
public class EditNewWords extends Activity {
	...
	private String user_review_table;
			...
			public void onClick(View arg0) {
				...
				for (int i = 0; i < select_words.size(); i++) {
					c = MainLfqActivity.getAcrosticsDb().rawQuery(
							"SELECT Information,Acrostics FROM "
					...
					if (c.moveToFirst()) {
						text += (i + 1)
								...
								+ select_words.get(i).getSelectedItem()
										.toString() + "<br />";
						text += "Information:<br />"
								+ c.getString(c.getColumnIndex("Information"))
								+ "<br />";
						text += "Acrostics:<br />"
								+ c.getString(c.getColumnIndex("Acrostics"))
								+ "<br />";
					}
					c.close();
				}
			...
			public void onClick(View arg0) {
				...
				if (check_change_review_times.isChecked()) {
					...
					sql = "UPDATE " + Helpers.db_prefix
							+ "newwords.user_review_times SET "
							+ sync_review_values + " WHERE UserName='"
							+ username + "'";
					// autoSync(sql, db, action, table, name, bool is_image,
					// byte[] image)
					autosync_text += Synchronize.autoSync(sql, "nw_db",
							"update", "user_review_times", username, false,
							null);
				...
				String usertable = username + "_savednewwords";
				...
				if (edit_newwords.isChecked()) {
					...
					for (int i = ct_usr_wors; i < number_words; i++) {
						...
						// image)
						autosync_text += Synchronize.autoSync(sql, "nw_db",
								"insert", user_review_table, "", false, null);
				...
				if (insert_newwords.isChecked()) {
					...
					if (c_ins.moveToFirst()) {
						...
						sql = "INSERT INTO " + Helpers.db_prefix + "newwords."
								+ usertable + " (Table_name,Word,Date) Values";
						sql2 = "INSERT INTO " + Helpers.db_prefix + "newwords."
								+ user_review_table
						...
						for (int i = 0; i < number_words; i++) {
							...
							cv.clear();
							cv.put("Table_name", select_tables.get(i)
									.getSelectedItem().toString());
							cv.put("Word", select_words.get(i)
									.getSelectedItem().toString());
							cv.put("Date", edidate);
							MainLfqActivity.getNewwordsDb().insert(usertable,
									null, cv);

							// INSERT INTO user_review_table:
							cv.clear();
							cv.put("Table_name", select_tables.get(i)
									.getSelectedItem().toString());
							cv.put("Word", select_words.get(i)
									.getSelectedItem().toString());
							MainLfqActivity.getNewwordsDb().insert(
									user_review_table, null, cv);
						...
						autosync_text += Synchronize.autoSync(sql, "nw_db",
								"insert", usertable, edidate, false, null);
						autosync_text += Synchronize.autoSync(sql2, "nw_db",
								"insert", user_review_table, edidate, false,
								null);
						results += ".";
						newword_results.setText(results + autosync_text);
					}
	...
	public void getAutoNewWords() {
		...
		user_review_table = username + "_savednewwords";
		for (int i = 1; i <= Integer.parseInt(select_number_newwords
			...
			TextView show_complete = new TextView(this_act);
			 ...
			 * is_one_table_clicked = false; return false; } });
			 * 
			 * select_table .setOnItemSelectedListener(new
			 * OnItemSelectedListener() {
			 * 
			 * @Override public void onItemSelected(AdapterView<?> arg0, View
			 * arg1, int arg2, long arg3) {
			 * 
			 * String table = select_table.getSelectedItem() .toString(); c =
			 * MainLfqActivity.getNewwordsDb().rawQuery("SELECT Word FROM " +
			 * user_review_table+" WHERE Table_name='"+table+"'", null);
			 * review_words.clear(); System.out.println("REVIEWED WORDS:"); int
			 * ct_rev_wor = 0; if (c.moveToFirst()) { do {
			 * review_words.add(c.getString(0)); ct_rev_wor++; } while
			 * (c.moveToNext()); } System.out.println(ct_rev_wor); c.close(); c
			 * = MainLfqActivity.getAcrosticsDb().rawQuery("SELECT Name FROM " +
			 * table + " WHERE Acrostics<>''", null); int ct_available_words =
			 * 0; wordsAdapter.clear(); Boolean is_reviewed=false; if
			 * (c.moveToFirst()) { do { is_reviewed=false; for (String str:
			 * review_words){ if
			 * (str.toLowerCase(Locale.ENGLISH).equals(c.getString
			 * (0).toLowerCase(Locale.ENGLISH))){
			 * System.out.println("rev="+str+", acr_wor="+c.getString(0));
			 * is_reviewed=true; break; } } if (!is_reviewed){
			 * ct_available_words++; wordsAdapter.add(c.getString(0)); } } while
			 * (c.moveToNext()); } Comparator<String> comparator = new
			 * Comparator<String>() {
			 * 
			 * @SuppressLint("DefaultLocale")
			 * 
			 * @Override public int compare(String str1, String str2) { return
			 * str1.toUpperCase(Locale.US) .compareTo(str2.toUpperCase()); } };
			 * wordsAdapter.sort(comparator);
			 * show_complete.setText(Html.fromHtml("<b>" + ct_available_words +
			 * " words</b>")); c.close(); }
			 * 
	...
	private void setReviewAdapter() {
		...
		Cursor c_reviews = MainLfqActivity.getNewwordsDb().rawQuery(
				"SELECT * FROM user_review_times WHERE UserName='" + username
						+ "'", null);
		if (c_reviews.moveToFirst()) {
			review_array = new String[] { "Time1", "Time2", "Time3", "Time4",
	...
	public void getWords() {
		...
		Cursor c_get = MainLfqActivity.getNewwordsDb().rawQuery(
				"SELECT * FROM " + usertable + " WHERE Date='" + edidate + "'",
				null);
		if (c_get.moveToFirst()) {
			do {
				...
				if (c_get_acrostics.moveToFirst()) {
					results += ct_get
							+ ")"
							+ c_get.getString(c_get.getColumnIndex("Word"))
							+ "<br />"
							+ " INFORMATION:<br />"
	...
	class doSetOneTables extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			if (check_all_same_table.isChecked()) {
				...
				sql = "SELECT DISTINCT Word FROM " + user_review_table
				...
				c = MainLfqActivity.getAcrosticsDb().rawQuery(
						"SELECT DISTINCT Name FROM " + my_table
								+ " WHERE Acrostics<>'' AND Name NOT IN ('"
								+ rev_str + "')", null);
