	...
	public void setListeners() {
		...
		make_list.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (Helpers.contains(cols, "Mnemonics")
						&& check_mnemonics.isChecked()) {
				...
				if (Helpers.contains(cols, "Peglist")
						&& check_peglist.isChecked()) {
				...
				if (Helpers.contains(cols, "Image")) {
					hasImg = true;
				}
		...
		protected String doInBackground(String... params) {
			...
			if (c_sel.moveToFirst()) {
				do {
					...
					mnevar = "";
					String name = c_sel.getString(c_sel.getColumnIndex("Name"));
					if (hasMne) {
					...
					if (check_images.isChecked()) {
						...
						c_get_img = MainLfqActivity.getAcrosticsDb().rawQuery(
								"SELECT Image From " + selected_table
					...
					Boolean isComplete = !(check_complete_information
							...
							c_sel.getColumnIndex("Information")).equals(""))
							&& !(check_complete_acrostics.isChecked() && c_sel
									.getString(
											c_sel.getColumnIndex("Acrostics"))
									.equals(""))
							...
							&& !(check_complete_peglist.isChecked() && !c_sel
									.getString(c_sel.getColumnIndex("Peglist"))
									.equals(""));
					...
					if (isComplete && isIncomplete) {
						...
						if (check_information.isChecked()) {
							...
							if (!c_sel.getString(
									c_sel.getColumnIndex("Information"))
									.equals("")) {
						...
						if (hasMne) {
							if (check_mnemonics.isChecked()
									&& !c_sel.getString(
											c_sel.getColumnIndex("Mnemonics"))
											.equals("")) {
