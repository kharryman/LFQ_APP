		...
		do {
			...
			try {				
				...
				for (int j = 0; j < ct_sqls; j++) {
					...
					if (!json.getString(acrostics.Image + j).equals("***NO_IMAGE***")){
						cv.clear();
						byte[] imageInByte = Base64.decode(json.getString(acrostics.Image + j),
								0);
						cv.put(acrostics.Image, imageInByte);
						database.update(table, cv, acrostics.Name+"=?", new String[]{json.getString(acrostics.Name + j)});
					}
				}
