...
public class EditAcrostics extends Activity {
	...
	public void setListeners() {
		...
		edit_acrostic.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (c.moveToFirst()) {
					exists = true;
					if (c.getColumnIndex(acrostics.Mnemonics) != -1) {
						mnemonic = mnemonic_text.getText().toString();
						mnemonic = mnemonic.replace("'", "`");
						mnemonic = mnemonic.replace("\"", "`");
					}
					if (c.getColumnIndex(acrostics.Peglist) != -1) {
				...
				if (insert_acrostics.isChecked()) {
					...
					if (c.moveToFirst()) {
						do {
							if (c.getString(c.getColumnIndex(acrostics.Name)) != null) {
								if (c.getString(c.getColumnIndex(acrostics.Name))
										.equals(name)) {
					...
					values.clear();
					values.put(acrostics.Name, name);
					values.put(acrostics.Information, info);
					values.put(acrostics.Acrostics, acrostic);
					sql = "INSERT INTO " + Helpers.db_prefix + tables.acrostics + "."
							+ table_name + " (" + acrostics.Name+ "," + acrostics.Information + "," + acrostics.Acrostics;
					String sql_end = ") VALUES('" + name + "','" + info + "','"
							+ acrostic + "'";
					if (c.getColumnIndex(acrostics.Mnemonics) != -1) {
						values.put(acrostics.Mnemonics, mnemonic);
						sql += ", " + acrostics.Mnemonics;
						sql_end += ",'" + mnemonic + "'";
					}
					if (c.getColumnIndex(acrostics.Peglist) != -1) {
						values.put(acrostics.Peglist, peglist);
						sql += ", " + acrostics.Peglist;
						sql_end += ",'" + peglist + "'";
					}
					String input = "";
					col_list.clear();
				...
				if (edit_acrostics.isChecked()) {
					...
					if (!exists) {
						...
						values.put(acrostics.Name, new_name);
						values.put(acrostics.Information, info);
						values.put(acrostics.Acrostics, acrostic);
						sql = "UPDATE " + Helpers.db_prefix + tables.acrostics + "."
								+ table_name + " SET " + acrostics.Name + "='" + new_name
								+ "', " + acrostics.Information + "='" + info + "', " + acrostics.Acrostics + "='"
								+ acrostic + "'";
						if (c.getColumnIndex(acrostics.Mnemonics) != -1) {
							values.put(acrostics.Mnemonics, mnemonic);
							sql += ", " + acrostics.Mnemonics + "='" + mnemonic + "'";
						}
						if (c.getColumnIndex(acrostics.Peglist) != -1) {
							values.put(acrostics.Peglist, peglist);
							sql += ", " + acrostics.Peglist + "='" + peglist + "'";
						}
						col_list.clear();
						col_list = getColumnTypes();
						for (int i = 0; i < col_list.size(); i++) {
						...
						MainLfqActivity.getAcrosticsDb().update(table_name,
								values, acrostics.Name + "=?", new String[] { name });
						sql += " WHERE " + acrostics.Name + "='" + name + "'";
						// autoSync(sql, db, action, table, name, bool is_image,
						// byte[] image)
						autosync_text = Synchronize.autoSync(sql, "acr_db",
				...
				if (delete_acrostics.isChecked()) {
					if (c.moveToFirst()) {
						MainLfqActivity.getAcrosticsDb().execSQL(
								"DELETE FROM " + table_name + " WHERE " + acrostics.Name + "='"
										+ name + "'");
						sql = "DELETE FROM `" + Helpers.db_prefix
								+ tables.acrostics + "`.`" + table_name
								+ "` WHERE " + acrostics.Name + "='" + name + "'";
						// autoSync(sql, db, action, table, name, bool is_image,
						// byte[] image)
						autosync_text = Synchronize.autoSync(sql, "acr_db",
		...
		get_acrostic.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (opt.equals("get_next")) {
					c = MainLfqActivity.getAcrosticsDb().rawQuery(
							"SELECT " + col_list_str + " FROM " + table_name
									+ " WHERE LOWER(" + acrostics.Name + ")>'"
									+ name.toLowerCase(Locale.US)
									+ "' ORDER BY LOWER(" + acrostics.Name + ") LIMIT 1", null);
				...
				if (opt.equals("get_last")) {
					c = MainLfqActivity.getAcrosticsDb().rawQuery(
							"SELECT " + col_list_str + " FROM " + table_name
									+ " WHERE LOWER(" + acrostics.Name + ")<'"
									+ name.toLowerCase(Locale.US)
									+ "' ORDER BY LOWER(" + acrostics.Name + ") DESC LIMIT 1",
							null);
					System.out.println("get last " + c.getColumnCount());
				}
				if (c.moveToFirst()) {
					if (opt.equals("get_last") || opt.equals("get_next")) {
						name = c.getString(c.getColumnIndex(acrostics.Name));
						input_acrostic_name.setText(name);
					}
	...
	public void getDictionaryWord() {
		...
		if (c.moveToFirst()) {
			if (opt.equals("get_last") || opt.equals("get_next")) {
				name = c.getString(c.getColumnIndex(dictionarya.Word));
				input_acrostic_name.setText(name);
			}
			doShowExists(name);
			...
			results.setText("Found " + name);
			information_text
					.setText(c.getString(c.getColumnIndex(dictionarya.Definition)));
	...
	public void doShowExists(String myName) {
		...
		c2 = MainLfqActivity.getAcrosticsDb()
				.rawQuery(
						"SELECT Name FROM " + table_name + " WHERE " + acrostics.Name + "='"
								+ name + "'", null);
	...
	public boolean isExist(String myName) {
		Cursor my_c = MainLfqActivity.getAcrosticsDb().rawQuery(
				"SELECT " + acrostics.Name + " FROM " + table_name + " WHERE " + acrostics.Name + "='" + myName
						+ "'", null);
		boolean ret = false;
		if (my_c.getCount() > 0) {
			ret = true;
	...
	public void getImage() {
		...
		c = MainLfqActivity.getAcrosticsDb().rawQuery(
				"SELECT " + acrostics.Image + " FROM " + table_name + " WHERE " + acrostics.Name + "='" + name
						+ "'", null);
		System.out.println("SELECT " + acrostics.Image + " FROM " + table_name + " WHERE " + acrostics.Name + "='"
				+ name + "'");
		if (c.getCount() > 0) {
			try {
				if (c.moveToFirst()) {
	...
	public List<String> getColumnsNotImage() {
		...
		for (int i = 0; i < col_names.length; i++) {
			if (!col_names[i].equals("android_metadata")
					...
					&& !col_names[i].equals(acrostics.Has_Image)) {
	...
	public List<String> getColumnTypes() {
		col_list.clear();
		c2 = MainLfqActivity.getAcrosticsDb().rawQuery(
				"SELECT * FROM " + table_name + " WHERE " + acrostics.Name + "='" + name + "'",
				null);
		...
		for (int i = 0; i < cols.length; i++) {
			if (!cols[i].equals("android_metadata")
					...
					&& !cols[i].equals("_id") && !cols[i].equals(acrostics.Name)
					&& !cols[i].equals(acrostics.Information)
					&& !cols[i].equals(acrostics.Acrostics) && !cols[i].equals(acrostics.Image)
	...
	public void refreshTables() {
		...
		if (col_list.size() > 0) {
			...
			for (int i = 0; i < col_list.size(); i++) {
				if (col_list.get(i).equals(acrostics.Mnemonics)) {
					mne_layout.setVisibility(View.VISIBLE);
					input_fields.set(2, true);
				}
				if (col_list.get(i).equals(acrostics.Peglist)) {
	...
	public void insertImage(byte[] myImage) throws IOException {
		if (myImage != null) {
			autosync_text = "";
			values.clear();
			values.put(acrostics.Image, myImage);
			values.put(acrostics.Has_Image, "yes");
			table_name = select_acrostic_tables.getSelectedItem().toString();
			MainLfqActivity.getAcrosticsDb().update(table_name, values,
					acrostics.Name + "='" + input_acrostic_name.getText().toString() + "'",
					null);
			// SYNC TO LFQ DATABASE:
			sql = "UPDATE " + Helpers.db_prefix + "acrostics."
					+ select_acrostic_tables.getSelectedItem().toString()
					+ " SET " + acrostics.Has_Image + "='yes' WHERE " + acrostics.Name + "='"
	...
	class doLoadWords extends AsyncTask<String, String, String> {
		...
		@Override
		protected String doInBackground(String... params) {
			// System.out.prin+tln("s=" + s + ", start=" + start + ", count="+
			// after);
			...
			if (check_use_dictionary.isChecked()) {
					...
					do {
						if (!c_tables.getString(0).equals("android_metadata")
							...
							Cursor c_1_table = MainLfqActivity.getAcrosticsDb()
									.rawQuery(
											"SELECT " + acrostics.Name + " FROM "
													+ c_tables.getString(0)
													+ " WHERE " + acrostics.Name + " LIKE '"
													+ name + "%'", null);
				...
				c2 = MainLfqActivity
						...
						.rawQuery(
								"SELECT " + acrostics.Name + " FROM "
										+ table_name
										+ " WHERE " + acrostics.Name + " LIKE ? ORDER BY " + acrostics.Name + " COLLATE NOCASE",
								selectionArgs);
			}
			if (!check_use_all_acrostics.isChecked()
